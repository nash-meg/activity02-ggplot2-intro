...
**Response**: The Adelie penguins have the shortest average flipper length while the Gentoo penguins have the longest flipper length on average.
...
**Response**: It is much easier to see the IQR and median on the boxplots, but the jitterplots show how many of each penguin species there are.
...
**Response**: I think the .md file is much easier to read and looks more professional. The .Rmd file looks like fresh code whereas the .md file looks complete and presentable.
...
**Response**: I prefer the jitterplot laid over the boxplots because then it seems like when you have a slope line under a scatterplot; the boxplots guide the jitterplots and help you identify trends more.
...
**Response**: When you map in the geom_boxplot layer, it creates a key on the right showing which color is which (in addition to the species already noted on the y axis)
...
**Response**: I think species is contributing most to the two peaks since the box plots for that variable are centered around 39 and 46, where as the sex and island boxplots are pretty spread out across the x axis (not clumped over certian values)
...
**Response**: As body mass (in grams) increases, the flipper length (in mm) also increases; the association is positive.
...
**Response**: The Gentoo species is by far the highest on average in body mass and flipper length. On the other hand, the Adelie and Chinstrap species are only on the bottom left of the graph with smaller body masses and flipper length on average.
...
**Response**: Making the points larger and slightly transparent helped to easier see where trends are. This is especially evident in the very center of the graph where it transitions from the green/red to the blue points as the smaller points were easy to miss.
